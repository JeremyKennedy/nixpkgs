...
in {
  ...
  # Check old behavior - argv[0] is the absolute path of the foreign binary
  dontPreserveArgvZero = makeTest {
    name = "systemd-binfmt-dont-preserve-argv0";
    machine = {
      boot.binfmt.emulatedSystems = [
        "aarch64-linux"
      ];
      boot.binfmt.registrations.aarch64-linux.preserveArgvZero = false;
    };
    testScript = let
      testAarch64 = expectArgv0 pkgs.pkgsCross.aarch64-multiplatform;
    in ''
      machine.start()
      machine.succeed("exec -a meow ${testAarch64} ${testAarch64}")
    '';
  };
}
