{ lib, stdenv, fetchurl, writeText, alsaLib, dbus, libGL, libpulseaudio, libva
, openssl, udev, xorg }:

let
  parsecd_json = builtins.fromJSON (builtins.readFile ./parsecd.json);
  appdata_json = let
    filtered = lib.filterAttrs (k: v: k != "nix_hash") parsecd_json;
  in writeText "appdata.json" (builtins.toJSON filtered);
in stdenv.mkDerivation {
  ...
  parsecd_so = fetchurl {
    url = "https://builds.parsecgaming.com/channel/release/binary/linux/gz/${parsecd_json.so_name}";
    sha256 = parsecd_json.nix_hash;
  };

  postPatch = ''
    cp ${appdata_json} usr/share/parsec/skel/appdata.json
    cp $parsecd_so usr/share/parsec/skel/${parsecd_json.so_name}
  '';
